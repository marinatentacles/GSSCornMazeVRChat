...
Shader "Fake Light/Point Light"
{
	...
	Subshader
	{
		Tags { "Queue"="Transparent-197" "ForceNoShadowCasting"="True" "IgnoreProjector"="True" "DisableBatching"="True" }
		LOD 100
		Cull[_CullMode]
		Blend One One
		...
		Pass
		{
			...
			fixed4 pixel_shader(v2f ps ) : SV_TARGET
			{
				...
				float finalLight = saturate(renderFakeLight(baseWorldPos, viewDirection, depthPosition));
				float3 finalColor = finalLight * ps.color * ps.color.a;
